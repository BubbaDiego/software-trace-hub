"""Resource Excel importer — parses FY26 Infusion resources.xlsm into rtm.db.
...
def import_resource_excel(
    ...
    if is_pivot:
        ...
        # Headers row 0 has summary info, actual headers might be row 2
        # Try to find monthly date columns
        month_cols = []
        ...
        # Skip header rows that are summaries
        start_row = 1
        if ci_name is None:
            # Try row 2 as header
