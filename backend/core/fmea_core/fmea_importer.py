"""FMEA Excel importer — parses Alaris-style SW FMEA spreadsheets into the shared DB.
...
def import_fmea_excel(
        ...
        if not target:
            for name in wb.sheetnames:
                if "fmea" in name.lower():
                    ...
                    break
        if not target:
            raise ValueError(f"No FMEA sheet found. Sheets: {wb.sheetnames}")
    ...
    if len(rows) < 4:
        raise ValueError("Sheet has fewer than 4 rows (need header + data)")
